...
export const Timer:React.FC<{seconds:number;onExpire:()=>void}> = ({seconds,onExpire}) => {
  ...
  const pct=(left/seconds)*100; const color=left>20?"#6fcf6f":left>10?"#e6b84a":"#ff7070";
  return (
    <div style={{display:"flex",alignItems:"center",gap:8}}>
      <span style={{fontSize:13,fontWeight:500,color,minWidth:32,textAlign:"right"}}>{left}s</span>
      <div style={{flex:1,height:4,background:"#2e2e2e",borderRadius:2,overflow:"hidden"}}><div style={{height:4,width:`${pct}%`,background:color,borderRadius:2,transition:"width 1s linear"}}/></div>
    </div>
  );
};
