...
export const ProfileStats:React.FC<ProfileStatsProps> = ({profile,rank,earnedCount,totalBadges}) => {
  const cards:StatCard[] = [
    { label:"Total XP", val:formatXP(profile.xp), icon:"zap", color:"#E85D24" },
    { label:`${profile.specialty} rank`, val:rank!=null?`#${rank}`:"—", icon:"trophy", color:"#e6b84a" },
  ...
  return (
    <div style={{display:"grid",gridTemplateColumns:"repeat(4, minmax(0,1fr))",gap:12,marginBottom:18}}>
      {cards.map(card=>(
        <div key={card.label} style={{position:"relative",background:"#1a1a1a",border:"0.5px solid #2e2e2e",borderLeft:`3px solid ${card.color}`,borderRadius:8,padding:"14px 16px",overflow:"hidden"}}>
          ...
          <div style={{position:"relative",display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
            ...
            <p style={{fontSize:12,color:"#999",margin:0}}>{card.label}</p>
          </div>
