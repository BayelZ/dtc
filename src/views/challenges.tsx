...
export const ChallengesPage:React.FC<{profile:Partial<Profile>|null;onXP:(xp:number)=>void}> = ({onXP}) => {
  ...
  const FB=({val,label,current,set}:{val:string;label:string;current:string;set:(v:string)=>void}) => (
    <button onClick={()=>set(val)} style={{padding:"6px 14px",fontSize:12,border:`0.5px solid ${current===val?"var(--accent)":"var(--border)"}`,borderRadius:6,background:current===val?"var(--accent-tint)":"var(--bg-card)",color:current===val?"var(--accent-hi)":"var(--text-muted)",cursor:"pointer"}}>{label}</button>
  );
  ...
  return (
    <div>
      ...
      {loading ? <p style={{color:"var(--text-muted)"}}>Loading challenges…</p> : (
        <div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:14}}>
          {filtered.map(c=><ChallengeCard key={c.id} challenge={c} completed={completed.has(c.id)} onClick={()=>setActive(c)} />)}
          {filtered.length===0 && <p style={{gridColumn:"1/-1",textAlign:"center",color:"var(--text-muted)",padding:"2rem"}}>No challenges match this filter.</p>}
        </div>
      )}
    </div>
  );
